...
export async function createFastify(): Promise<FastifyInstance> {
  ...
  const mod = (await import('../../src/app.js')) as unknown;
  const buildApp = (mod as { buildApp: () => Promise<FastifyInstance> }).buildApp;
...
export async function createRole(data: { name: string; tenantId: Types.ObjectId }) {
...
export async function createUser(data: { tenantId: Types.ObjectId }) {
...
export async function createUserRole(data: { userId: string; roleId: string; tenantId: Types.ObjectId }) {
...
export function generateJwt(user: { id?: string; _id?: string; username?: string; email?: string; role?: any }) {
